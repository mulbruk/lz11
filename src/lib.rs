...
const LZ11_MIN_REPEAT_LENGTH: usize = 3;
const LZ11_MAX_REPEAT_LENGTH: usize = 65808; // (2^16 - 1) + 0x111
...
fn naive_find_longest_match(data: &[u8], offset: usize) -> Option<(usize, usize)> {
  ...
  for n in match_start..offset {
    ...
    while offset + match_length < data.len() && match_length < LZ11_MAX_REPEAT_LENGTH && data[n + match_length] == data[offset + match_length] {
      match_length += 1;
    }
  ...
  if longest_match < LZ11_MIN_REPEAT_LENGTH {
...
pub fn compress_lz11(data: &[u8]) -> Result<Vec<u8>, LZError> {
